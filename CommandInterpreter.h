...
public:
    ...
    public:
        ...
        char const * alias(unsigned int i) { return (i < mAliasCount) ? mAlias[i] : nullptr; }
        unsigned int aliasCount() { return mAliasCount; }
        char const * argument(unsigned int i) { return (i < mArgumentCount) ? mArgument[i] : nullptr; }
    ...
    CommandInterpreter(Stream& serial);
    ~CommandInterpreter();
